...
/**
 * \def TOUCHTOUCH
 * \brief A macro to explicitly configure use with the Duinopeak's TouchTouch Board
 *
 * Duinopeak's TouchTouch is supported. However, its pin mapping is 
 ...
 * as follows:
 *
 * 1. Download the Duinopeak's TouchTouch board definitions file (boards.txt) from
 *    their Github (https://github.com/duinopeak).
 * 2. Extract the TouchTouch folder into your Documents/Arduino/Hardware
 ...
 * 4. In the Arduino menu, select Tools -> Board -> TouchTouch
 * 
 ...
 * would like to override this, set the TOUCHTOUCH value below: 0 to use the
 * Sparkfun MP3 shield, 1 to use the Duinopeak's TouchTouch Board.
 *
 * If you are using an earlier version of Arduino, you will have to manage the
 * pin remapping manually. Setting TOUCHTOUCH below to 0 will leave the pin map
 * as normal - i.e. for the Sparkfun MP3 shield. Setting it to 1 will map the 
 * pins correctly for the Duinopeak's TouchTouch Board. If you decide to then
 * use a different board, you'll have to remember to come back here and adjust 
 * the settings accordingly.
 * 
 */
#define TOUCHTOUCH 0 // set to 1 to force Duinopeak's TouchTouch Board settings on
...
// if TOUCHTOUCH or ARDUINO_AVR_TOUCHTOUCH use the following pin map
#elif (( TOUCHTOUCH == 1 ) || ( ARDUINO_AVR_TOUCHTOUCH == 1 )) 	
